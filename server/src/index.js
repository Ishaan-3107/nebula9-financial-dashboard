...
wss.on("connection", (ws) => {
  registerClient(ws);
  ws.send(
    JSON.stringify({ type: "hello", payload: { message: "stream connected" } }),
  );
});
...
async function main() {
  ...
  if (!config.finnhubApiKey) {
    console.warn(
      "FINNHUB_API_KEY is not configured. Live Finnhub quotes will be disabled and simulated data will be used instead.",
    );
  }
